...
class MM(nn.Module):
    def __init__(self,training,transforms,loss_names,checkpoint=r'/data/meng/meng/OOD/vt_300/ckpts/model_110.pth',device='cuda:0',**kwargs):
        super().__init__()
    ...
    def _train(self, sample):
        ...
        loss.backward()
        # print("model get optim step")
        self.optim.step()
        # print("vt loss_trackers['loss']")
        # print(self.loss_trackers['loss'])
    ...
    def _evaluate(self, sample):
        ...
        output_flip = np.concatenate([azimuth[:, ::-1], elevation, theta[:, ::-1]], axis=1).reshape(-1,
                                                                                                    self.num_bins * 3)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            self, img_size=256, patch_size=16, in_chans=3, num_classes=123, global_pool='token',
        ...
        self.encoder= nn.TransformerEncoderLayer(1024,4,1024,0.5)
    ...
    def forward_head(self, x, pre_logits: bool = False,istrain: bool = False):
        if self.global_pool:
    ...
    def forward(self, x,istrain: bool = False):
        x = self.forward_features(x)
        x = self.forward_head(x,istrain = istrain)
        return x
